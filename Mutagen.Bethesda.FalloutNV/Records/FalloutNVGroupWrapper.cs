...
namespace Mutagen.Bethesda.FalloutNV.Records;

internal class FalloutNVGroupWrapper<TMajor> : IFalloutNVGroupGetter<TMajor>
    where TMajor : class, IFalloutNVMajorRecordGetter, IBinaryItem
{
    private readonly GroupMergeGetter<IFalloutNVGroupGetter<TMajor>, TMajor> _groupMerge;

    public FalloutNVGroupWrapper(GroupMergeGetter<IFalloutNVGroupGetter<TMajor>, TMajor> groupMerge)
    {
    ...
    public static FalloutNVGroup_Registration StaticRegistration => FalloutNVGroup_Registration.Instance;

    [DebuggerStepThrough]
    protected object CommonInstance(Type type0) =>
        GenericCommonInstanceGetter.Get(FalloutNVGroupCommon<TMajor>.Instance, typeof(TMajor), type0);

    [DebuggerStepThrough]
    protected object CommonSetterTranslationInstance() => FalloutNVGroupSetterTranslationCommon.Instance;

    [DebuggerStepThrough]
    object IFalloutNVGroupGetter<TMajor>.CommonInstance(Type type0) => this.CommonInstance(type0);

    [DebuggerStepThrough]
    object? IFalloutNVGroupGetter<TMajor>.CommonSetterInstance(Type type0) => null;

    [DebuggerStepThrough]
    object IFalloutNVGroupGetter<TMajor>.CommonSetterTranslationInstance() => this.CommonSetterTranslationInstance();
