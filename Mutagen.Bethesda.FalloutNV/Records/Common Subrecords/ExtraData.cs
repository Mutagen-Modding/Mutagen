...
partial class ExtraDataBinaryCreateTranslation
{
    public static OwnerTarget GetBinaryOwner(ReadOnlySpan<byte> span, RecordTypeInfoCacheReader cache, IReadOnlySeparatedMasterPackage masters)
    {
        ...
        {
            OwnerData = new FormLink<IFalloutNVMajorRecordGetter>(FormKeyBinaryTranslation.Instance.Parse(span, masters)),
            VariableData = new FormLink<IFalloutNVMajorRecordGetter>(FormKeyBinaryTranslation.Instance.Parse(span.Slice(4), masters))
        };
    }
...
partial class ExtraDataBinaryOverlay
{
    ...
    public IOwnerTargetGetter GetOwnerCustom(int location) => ExtraDataBinaryCreateTranslation.GetBinaryOwner(_structData.Slice(location), _package.MetaData.RecordInfoCache!, _package.MetaData.MasterReferences);
}
