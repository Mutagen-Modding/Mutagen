...
namespace Mutagen.Bethesda.Generation
{
    public class GenericsModule : GenerationModule
    {
        ...
        public override async Task GenerateInStaticCtor(ObjectGeneration obj, FileGeneration fg)
        {
            foreach (var genName in GetGenerics(obj, fg))
            {
                fg.AppendLine($"{genName}_RecordType = (RecordType)LoquiRegistration.GetRegister(typeof(T))!.GetType().GetField(Mutagen.Bethesda.Constants.{nameof(Mutagen.Bethesda.Constants.TriggeringRecordTypeMember)}).GetValue(null);");
            }
