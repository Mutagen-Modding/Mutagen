...
namespace Mutagen.Bethesda.Generation
{
    public class MutagenXmlModule : XmlTranslationModule
    {
            ...
            using (new BraceWrapper(fg))
            {
                if (obj.HasLoquiBaseObject)
                {
                    using (var args = new ArgsWrapper(fg,
                        $"{this.TranslationClass(obj.BaseClass)}.WriteToNode{ModuleNickname}"))
                    {
                        args.Add($"item: item");
        ...
        {
            using (var args = new FunctionWrapper(fg,
                $"public static void FillPublicElement{ModuleNickname}{obj.GetGenericTypes(MaskType.Normal)}",
            ...
            using (new BraceWrapper(fg))
            {
                ...
                using (new BraceWrapper(fg))
                {
                    ...
                    using (new DepthWrapper(fg))
                    {
                        ...
                        {
                            using (var args = new ArgsWrapper(fg,
                                $"{this.TranslationClass(obj.BaseClass)}.FillPublicElement{ModuleNickname}{obj.GetBaseMask_GenericTypes(MaskType.Error)}"))
                            {
                                args.Add("item: item");
                                args.Add($"{XmlTranslationModule.XElementLine.GetParameterName(obj)}: {XmlTranslationModule.XElementLine.GetParameterName(obj)}");
                                args.Add("name: name");
