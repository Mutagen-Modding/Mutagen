...
partial class NavigationMapInfoBinaryOverlay
{
    ...
    {
        var worldspace = FormKey.Factory(_package.MetaData.MasterReferences.Raw, BinaryPrimitives.ReadUInt32LittleEndian(_structData.Span.Slice(IslandEndingPos + 0x4, 0x4)));
        if (worldspace.IsNull)
        {
            return new NavigationMapInfoCellParent()
            {
                Cell = FormKey.Factory(_package.MetaData.MasterReferences.Raw, BinaryPrimitives.ReadUInt32LittleEndian(_structData.Span.Slice(IslandEndingPos + 0x8, 0x4))).ToLink<ICellGetter>()
            };
        }
        else
        {
