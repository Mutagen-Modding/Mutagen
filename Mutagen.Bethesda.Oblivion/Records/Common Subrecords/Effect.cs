    ...
    namespace Internals
    {
        public partial class EffectBinaryCreateTranslation
        {
            static partial void FillBinaryEffectInitialCustom(MutagenFrame frame, IEffect item)
            {
                ...
                if (subMeta.ContentLength != Mutagen.Bethesda.Constants.Constants.HeaderLength)
                {
                ...
                if (efitMeta.ContentLength < Mutagen.Bethesda.Constants.Constants.HeaderLength)
                {
                    throw new ArgumentException($"Magic effect ref length was less than 4.  Was: {efitMeta.ContentLength}");
                }
                var magicEffName2 = frame.GetMemory(amount: Mutagen.Bethesda.Constants.Constants.HeaderLength, offset: efitMeta.HeaderLength);
