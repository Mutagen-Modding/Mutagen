...
partial class Package
{
    ...
    public enum Group
    {
        ...
        FleeArea = 15,
        FollowPlayer = 17,
        TravelBetweenLandings = 20,
        SandboxEditorLocation = 21,
        Ambush = 22,
        EscortPlayer = 23,
        SandboxInteriorUnlockDoors = 27,
        KeepEyeOn = 29,
        Sleep = 30,
        DefaultPassengerPackage = 32,
        UseWeapon = 34,
        ForceGreetWhileAimWeapon = 35,
        SandboxWalk = 36,
        ForceGreet1 = 39,
        ForceGreet2 = 40,
        SuspiciousPatrol = 41,
        AmbushAtFoundLocation = 42,
        PatrolCreature = 43,
        UseMagicAndWait = 45,
        SleepAtLair = 46,
        AnimTestPatrol = 49,
        UseMagicAndWaitNoTravel = 50,
        AnimTestSandboxPackage = 53,
        ForceGreetPatrolGuard = 55,
        PredatorSeekPrey = 57,
        FollowPlayer2 = 60,
        RangingWithPlayer = 66,
        DefaultCombatMasterTemplate = 81,
        MasterShootTarget = 134,
        ShootTarget = 145,
    }
...
partial class PackageBinaryCreateTranslation
{
        ...
        while (itemIndex < expectedCount)
        {
            ...
            switch (subRecord.RecordTypeInt)
            {
                ...
                case RecordTypeInts.STSC:
                {
                    ...
                    if (lastPackage is PackageDataScene scene)
                    {
                        stream.Position += subRecord.HeaderLength;
                        scene.Scene.SetTo(FormKeyBinaryTranslation.Instance.Parse(stream));
                    }
                    else
                    {
                        throw new ArgumentException("Encountered target for a non-target data package");
                ...
                case RecordTypeInts.PLDT:
                    ...
                    {
                        stream.Position += subRecord.HeaderLength;
                        loc.Location = Mutagen.Bethesda.Starfield.LocationTargetRadius.CreateFromBinary(new MutagenFrame(stream).SpawnWithLength(subRecord.ContentLength));
                    }
                    else
                    {
    ...
    public static partial void FillBinaryProcedureTreeCustom(MutagenFrame frame, IPackageInternal item, PreviousParse lastParsed)
    {
...
partial class PackageBinaryWriteTranslation
{
    public static partial void WriteBinaryPackageTemplateCustom(MutagenWriter writer, IPackageGetter item)
    {
        ...
        foreach (var kv in data.OrderBy(k => k.Key))
        {
            ...
            var typeStr = val switch
            {
                ...
                PackageDataScene => PackageBinaryCreateTranslation.SceneKey,
                _ => null,
            };
            ...
            switch (val)
            {
                ...
                case PackageDataLocation loc:
                    ...
                    break;
                case PackageDataScene scene:
                    FormLinkBinaryTranslation.Instance.WriteNullable(writer, scene.Scene, RecordTypes.STSC);
                    break;
                default:
                    throw new NotImplementedException();
            }
    ...
    public static partial void WriteBinaryProcedureTreeCustom(MutagenWriter writer, IPackageGetter item)
    {
...
partial class PackageBinaryOverlay
{
        ...
        int offset)
    {
        ...
        _packageTemplate = FormKeyBinaryTranslation.Instance.Parse(pkcu.Content.Slice(4), _package.MetaData.MasterReferences.Raw);
