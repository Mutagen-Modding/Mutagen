...
namespace Mutagen.Bethesda.Starfield
{
        ...
        IRaceInternal
    {
        ...
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
        IVirtualMachineAdapterGetter? IRaceGetter.VirtualMachineAdapter => this.VirtualMachineAdapter;
        #region Aspects
            ...
            IErrorMask<ErrorMask>
        {
            ...
            public override object? GetNthMask(int index)
            {
                ...
                switch (enu)
                {
                    case Race_FieldIndex.VirtualMachineAdapter:
...
namespace Mutagen.Bethesda.Starfield
{
    ...
    internal enum Race_FieldIndex
    {
        ...
        Debris = 29,
        ImpactDataSet = 30,
        OrientationLimitsPitch = 31,
        OrientationLimitsRoll = 32,
        DAT2Unknown3 = 33,
        SkeletalModel = 34,
        MovementTypeNames = 35,
        Voices = 36,
        FacegenMainClamp = 37,
        FacegenFaceClamp = 38,
        Attacks = 39,
        ...
        WED1 = 44,
        BipedObjects = 45,
        MovementDataOverrides = 46,
        EquipmentFlags = 47,
        EquipmentSlots = 48,
        UnarmedWeapon = 49,
        BaseMovementDefault = 50,
        BaseMovementSwimDefault = 51,
        BaseMovementFlyDefault = 52,
        ChargenAndSkintones = 53,
        ArmorRace = 54,
        SubgraphTemplateRace = 55,
        Subgraphs = 56,
        IdleChatterTimeMin = 57,
        IdleChatterTimeMax = 58,
        DialogueQuest = 59,
        HeadPartsAndBoneModifiers = 60,
        MannequinSkinSwaps = 61,
        PluralName = 62,
    }
    #endregion
    ...
    internal partial class RaceCommon : StarfieldMajorRecordCommon
    {
        ...
        public IEnumerable<IFormLinkGetter> EnumerateFormLinks(IRaceGetter obj, bool iterateNestedRecords = true)
        {
            ...
            if (obj.SkeletalModel is {} SkeletalModelItem)
            {
                foreach (var item in SkeletalModelItem.WhereNotNull().SelectMany(f => f.EnumerateFormLinks(iterateNestedRecords)))
                {
                    yield return FormLinkInformation.Factory(item);
                }
            }
            ...
            foreach (var item in obj.BipedObjects.Values.SelectMany(f => f.EnumerateFormLinks(iterateNestedRecords)))
            {
                yield return item;
            }
            foreach (var item in obj.MovementDataOverrides.SelectMany(f => f.EnumerateFormLinks(iterateNestedRecords)))
            {
                yield return FormLinkInformation.Factory(item);
            }
            foreach (var item in obj.EquipmentSlots.SelectMany(f => f.EnumerateFormLinks(iterateNestedRecords)))
            {
                yield return FormLinkInformation.Factory(item);
            }
            ...
            {
                foreach (var item in ChargenAndSkintonesItem.WhereNotNull().SelectMany(f => f.EnumerateFormLinks(iterateNestedRecords)))
                {
                    yield return FormLinkInformation.Factory(item);
                }
            }
            ...
            foreach (var item in obj.Subgraphs.SelectMany(f => f.EnumerateFormLinks(iterateNestedRecords)))
            {
                yield return FormLinkInformation.Factory(item);
            }
            ...
            {
                foreach (var item in HeadPartsAndBoneModifiersItem.WhereNotNull().SelectMany(f => f.EnumerateFormLinks(iterateNestedRecords)))
                {
                    yield return FormLinkInformation.Factory(item);
                }
            }
            foreach (var item in obj.MannequinSkinSwaps.SelectMany(f => f.EnumerateFormLinks(iterateNestedRecords)))
            {
                yield return FormLinkInformation.Factory(item);
            }
            yield break;
    ...
    internal partial class RaceSetterTranslationCommon : StarfieldMajorRecordSetterTranslationCommon
    {
            ...
            bool deepCopy)
        {
            base.DeepCopyIn(
                ...
                copyMask,
                deepCopy: deepCopy);
            if ((copyMask?.GetShouldTranslate((int)Race_FieldIndex.VirtualMachineAdapter) ?? true))
            {
                errorMask?.PushIndex((int)Race_FieldIndex.VirtualMachineAdapter);
                try
                {
                    if(rhs.VirtualMachineAdapter is {} rhsVirtualMachineAdapter)
                    {
                        item.VirtualMachineAdapter = rhsVirtualMachineAdapter.DeepCopy(
                            errorMask: errorMask,
                            copyMask?.GetSubCrystal((int)Race_FieldIndex.VirtualMachineAdapter));
                    }
                    else
                    {
            ...
            TypedParseParams translationParams = default)
        {
            ...
            switch (nextRecordType.TypeInt)
            {
                case RecordTypeInts.VMAD:
                {
                    ...
                    return (int)Race_FieldIndex.VirtualMachineAdapter;
                }
                ...
                {
                    frame.Position += frame.MetaData.Constants.SubConstants.HeaderLength; // Skip header
                    item.WED1 = Mutagen.Bethesda.Starfield.SoundReference.CreateFromBinary(frame: frame.SpawnWithLength(contentLength));
