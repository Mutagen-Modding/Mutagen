    ...
    where T : struct, Enum
{
        ...
        PreviousParse lastParsed)
    {
        ...
        item.LevelMin = frame.ReadUInt16();
        item.LevelMax = frame.ReadUInt16();
    ...
    public static AObjectModProperty<T> ReadProperty(IReadOnlySeparatedMasterPackage masters, ReadOnlySpan<byte> data)
    {
        ...
        var enumVal = data[4];
        var Prop = Enums<T>.Convert(BinaryPrimitives.ReadUInt32LittleEndian(data[8..]));
        switch (type)
        {
...
partial class ObjectTemplateBinaryWriteTranslation
{
        ...
        where TFunctionEnum : struct, Enum
    {
        ...
        writer.WriteZeros(3);
        writer.Write(checked((uint)Enums<TPropertyEnum>.ConvertFrom(prop.Property)));
    }
}
...
partial class ObjectTemplateBinaryOverlay<T>
{
    ...
    public ushort LevelMin => _recordData[_obtsLoc!.Value + 8];
    public ushort LevelMax => _recordData[_obtsLoc!.Value + 10];
