    ...
    public static partial class StarfieldModMixIn
    {
            ...
            IFileSystem? fileSystem = null)
        {
            ...
            var modKey = param.RunMasterMatch(
                mod: item,
                path: path);
            param.StringsWriter ??= Enums.HasFlag((int)item.ModHeader.Flags, (int)ModHeaderCommonFlag.Localized) ? new StringsWriter(GameRelease.Starfield, modKey, Path.Combine(Path.GetDirectoryName(path)!, "Strings"), MutagenEncoding.Default) : null;
