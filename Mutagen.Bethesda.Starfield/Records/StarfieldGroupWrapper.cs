...
namespace Mutagen.Bethesda.Starfield.Records;

internal class StarfieldGroupWrapper<TMajor> : IStarfieldGroupGetter<TMajor>
    where TMajor : class, IStarfieldMajorRecordGetter, IBinaryItem
{
    private readonly GroupMergeGetter<IStarfieldGroupGetter<TMajor>, TMajor> _groupMerge;

    public StarfieldGroupWrapper(GroupMergeGetter<IStarfieldGroupGetter<TMajor>, TMajor> groupMerge)
    {
    ...
    public static StarfieldGroup_Registration StaticRegistration => StarfieldGroup_Registration.Instance;

    [DebuggerStepThrough]
    protected object CommonInstance(Type type0) =>
        GenericCommonInstanceGetter.Get(StarfieldGroupCommon<TMajor>.Instance, typeof(TMajor), type0);

    [DebuggerStepThrough]
    protected object CommonSetterTranslationInstance() => StarfieldGroupSetterTranslationCommon.Instance;

    [DebuggerStepThrough]
    object IStarfieldGroupGetter<TMajor>.CommonInstance(Type type0) => this.CommonInstance(type0);

    [DebuggerStepThrough]
    object? IStarfieldGroupGetter<TMajor>.CommonSetterInstance(Type type0) => null;

    [DebuggerStepThrough]
    object IStarfieldGroupGetter<TMajor>.CommonSetterTranslationInstance() => this.CommonSetterTranslationInstance();
