...
public partial class Fallout76Mod : AMod
{
    public override uint GetDefaultInitialNextFormID(bool? forceUseLowerFormIDRanges = false) =>
        GetDefaultInitialNextFormIDStatic(this.Fallout76Release, this.ModHeader.Stats.Version, forceUseLowerFormIDRanges);
    
    public override bool IsMaster
    {
        get => this.ModHeader.Flags.HasFlag(Fallout76ModHeader.HeaderFlag.Master);
        set => this.ModHeader.Flags = this.ModHeader.Flags.SetFlag(Fallout76ModHeader.HeaderFlag.Master, value);
    }
    ...
    {
        get => this.ModHeader.Flags.HasFlag(Fallout76ModHeader.HeaderFlag.Small);
        set => this.ModHeader.Flags = this.ModHeader.Flags.SetFlag(Fallout76ModHeader.HeaderFlag.Small, value);
    }
    ...
    internal static uint GetDefaultInitialNextFormIDStatic(Fallout76Release release, float headerVersion, bool? forceUseLowerFormIDRanges)
    {
    ...
    internal class Fallout76CreateBuilderInstantiator : IBinaryReadBuilderInstantiator<IFallout76Mod, IFallout76ModDisposableGetter, GroupMask>
    {
        public static readonly Fallout76CreateBuilderInstantiator Instance = new();
        
        public IFallout76Mod Mutable(BinaryReadMutableBuilder<IFallout76Mod, IFallout76ModDisposableGetter, GroupMask> builder)
        {
            if (builder._param._path != null)
            {
                return Fallout76Mod.CreateFromBinary(
                    path: new ModPath(
                        builder._param.ModKey,
                        builder._param._path.Value),
                    release: builder._param.GameRelease.ToFallout76Release(),
        ...
        public IFallout76ModDisposableGetter Readonly(BinaryReadBuilder<IFallout76Mod, IFallout76ModDisposableGetter, GroupMask> builder)
        {
            if (builder._param._path != null)
            {
                return Fallout76Mod.CreateFromBinaryOverlay(
                    path: new ModPath(
                        builder._param.ModKey,
                        builder._param._path.Value),
                    release: builder._param.GameRelease.ToFallout76Release(),
    ...
    public static BinaryReadBuilderSourceStreamFactoryChoice<IFallout76Mod, IFallout76ModDisposableGetter, GroupMask> 
        Create(Fallout76Release release) => new( 
        release.ToGameRelease(), 
        Fallout76CreateBuilderInstantiator.Instance,
        needsRecordTypeInfoCacheReader: false);
    
    internal class Fallout76WriteBuilderInstantiator : IBinaryWriteBuilderWriter<IFallout76ModGetter>
    {
        public static readonly Fallout76WriteBuilderInstantiator Instance = new();
        private static readonly IAutoSplitModWriter AutoSplitter = new AutoSplitModWriter(new MultiModFileSplitter());

        public async Task WriteAsync(IFallout76ModGetter mod, BinaryWriteBuilderParams<IFallout76ModGetter> param)
        {
            Write(mod, param);
        }

        public void Write(IFallout76ModGetter mod, BinaryWriteBuilderParams<IFallout76ModGetter> param)
        {
            if (param._path != null)
            {
                if (param._autoSplit)
                {
                    AutoSplitter.Write<IFallout76Mod, IFallout76ModGetter>(
                        mod,
                        param._path.Value,
                        param._param);
                }
    ...
    public BinaryModdedWriteBuilderTargetChoice<IFallout76ModGetter> 
        BeginWrite => new(
        this, 
        Fallout76WriteBuilderInstantiator.Instance);

    IBinaryModdedWriteBuilderTargetChoice IModGetter.BeginWrite => this.BeginWrite;

    public static BinaryWriteBuilderTargetChoice<IFallout76ModGetter> WriteBuilder(Fallout76Release release) =>
        new(release.ToGameRelease(), Fallout76WriteBuilderInstantiator.Instance);
    
    IMod IModGetter.DeepCopy() => this.DeepCopy();
}

public partial interface IFallout76ModGetter
{
    BinaryModdedWriteBuilderTargetChoice<IFallout76ModGetter> BeginWrite { get; }
}

partial class Fallout76ModSetterTranslationCommon
{
    partial void DeepCopyInCustom(IFallout76Mod item, IFallout76ModGetter rhs, ErrorMaskBuilder? errorMask,
    ...
    public partial Fallout76Mod DeepCopyGetNew(IFallout76ModGetter item)
    {
        return new Fallout76Mod(item.ModKey, item.Fallout76Release);
    }
}

internal partial class Fallout76ModBinaryOverlay
{
    public uint GetDefaultInitialNextFormID(bool? forceUseLowerFormIDRanges = false) =>
        Fallout76Mod.GetDefaultInitialNextFormIDStatic(
            this.Fallout76Release,
            this.ModHeader.Stats.Version,
            forceUseLowerFormIDRanges);
    
    public bool IsMaster => this.ModHeader.Flags.HasFlag(Fallout76ModHeader.HeaderFlag.Master);
    public bool CanBeSmallMaster => true;
    public bool IsSmallMaster => this.ModHeader.Flags.HasFlag(Fallout76ModHeader.HeaderFlag.Small);
    ...
    IBinaryModdedWriteBuilderTargetChoice IModGetter.BeginWrite => 
        new BinaryModdedWriteBuilderTargetChoice<IFallout76ModGetter>(
            this, 
            Fallout76Mod.Fallout76WriteBuilderInstantiator.Instance);

    public BinaryModdedWriteBuilderTargetChoice<IFallout76ModGetter> BeginWrite => 
        new BinaryModdedWriteBuilderTargetChoice<IFallout76ModGetter>(
            this, 
            Fallout76Mod.Fallout76WriteBuilderInstantiator.Instance);
...
partial class Fallout76ModCommon
{
    public static void WriteCellsParallel(
        IFallout76ListGroupGetter<ICellBlockGetter> group,
        int targetIndex,
        ...
        ParallelWriteParameters parallelWriteParameters)
    {
        ...
        {
            stream.Position += 8;
            Fallout76ListGroupBinaryWriteTranslation.WriteEmbedded<ICellBlockGetter>(group, stream);
        }
    ...
    public static void WriteWorldspacesParallel(
        IFallout76GroupGetter<IWorldspaceGetter> group,
        int targetIndex,
        Stream[] streamDepositArray,
        WritingBundle bundle,
        ParallelWriteParameters parallelWriteParameters)
    {
        ...
        {
            stream.Position += 8;
            Fallout76GroupBinaryWriteTranslation.WriteEmbedded<IWorldspaceGetter>(group, stream);
        }
        streams[0] = groupByteStream;
    ...
    public static void WriteQuestsParallel(
        IFallout76GroupGetter<IQuestGetter> group,
        int targetIndex,
        Stream[] streamDepositArray,
        WritingBundle bundle,
        ParallelWriteParameters parallelWriteParameters)
    ...
    partial void GetCustomRecordCount(IFallout76ModGetter item, Action<uint> setter)
    {
        uint count = 0;
