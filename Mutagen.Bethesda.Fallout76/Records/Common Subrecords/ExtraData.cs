...
partial class ExtraDataBinaryCreateTranslation
{
    public static OwnerTarget GetBinaryOwner(ReadOnlySpan<byte> span, RecordTypeInfoCacheReader cache, IReadOnlySeparatedMasterPackage masters)
    {
        ...
        {
            OwnerData = new FormLink<IFallout76MajorRecordGetter>(FormKeyBinaryTranslation.Instance.Parse(span, masters)),
            VariableData = new FormLink<IFallout76MajorRecordGetter>(FormKeyBinaryTranslation.Instance.Parse(span.Slice(4), masters))
        };
    }
