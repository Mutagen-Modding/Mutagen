...
public class SkyrimProcessor : Processor
{
        ...
        long fileOffset)
    {
        ...
        if (!majorFrame.TryLocateSubrecordFrame(RecordTypes.ENAM, out var rec)) return;
    ...
    private void ProcessNpcs(
        ...
        long fileOffset)
    {
        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.QNAM, out var qnamFrame))
        {
        ...
        long fileOffset)
    {
        ...
        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.MHDT, out var pin))
        {
        ...
        long fileOffset)
    {
        ...
        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.TIFC, out var tifcRec))
        {
    ...
    private void ProcessQuests(
        ...
        long fileOffset)
    {
        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.ANAM, out var anamRec))
        {
    ...
    public void FixVMADFormIDs(
        ...
        out int processed)
    {
        vmad = PluginUtilityTranslation.FindFirstSubrecord(frame.Content, Meta, RecordTypes.VMAD);
        if (vmad == null)
        {
            processed = 0;
            objectFormat = 0;
        ...
        long fileOffset)
    {
        ...
        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.XTEL, out var xtelRec))
        {
            var offset = 4;
            ProcessZeroFloats(xtelRec, fileOffset, ref offset, 6);
        }

        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.XPRM, out var xprmRec))
        {
            int offset = 0;
        ...
        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.XRMR, out var xrmrRec))
        {
    ...
    private void ProcessNavmeshes(
        ...
        long fileOffset)
    {
        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.NVNM, out var nvnmRec))
        {
        ...
        long fileOffset)
    {
        ...
        // Reorder data values
        var xnamPos =
            PluginUtilityTranslation.FindFirstSubrecord(majorFrame.Content, majorFrame.Meta, RecordTypes.XNAM)?.Location;
        if (xnamPos == null)
        {
        ...
        var anamPos =
            PluginUtilityTranslation.FindFirstSubrecord(majorFrame.Content, majorFrame.Meta, RecordTypes.ANAM)?.Location;
        ...
        // Reorder inputs
        var unamPos =
            PluginUtilityTranslation.FindFirstSubrecord(majorFrame.Content.Slice(xnamPos.Value), majorFrame.Meta, unam)?.Location;
    ...
    private void ProcessLoadScreens(
        ...
        long fileOffset)
    {
        if (majorFrame.TryLocateSubrecordFrame(RecordTypes.XNAM, out var xnamRec))
        {
