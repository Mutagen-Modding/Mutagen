    ...
    internal static class GenderedItem
    {
            ...
            where TMask : class, IMask<bool>
        {
            if (lhs == null || rhs == null)
            {
                if (lhs == null && rhs == null)
                {
                    return include == Include.All
                        ? new MaskItem<bool, GenderedItem<MaskItem<bool, TMask?>?>?>(true, default)
                        : null;
                }
        ...
        public static bool AllMask<TItem, TMask>(MaskItem<TItem, GenderedItem<MaskItem<TItem, TMask?>?>?>? item,
            Func<TItem, bool> eval)
            where TMask : class, IMask<TItem>
        {
            if (item == null) return true;
            if (!eval(item.Overall)) return false;
        ...
        public static bool AnyMask<TItem, TMask>(MaskItem<TItem, GenderedItem<MaskItem<TItem, TMask?>?>?>? item,
            Func<TItem, bool> eval)
            where TMask : class, IMask<TItem>
        {
            if (item == null) return false;
        ...
        public static MaskItem<TRet, GenderedItem<TRet>?>? TranslateHelper<TItem, TRet>(
            MaskItem<TItem, GenderedItem<TItem>?>? mask, Func<TItem, TRet> eval)
        {
            if (mask == null)
            {
                return null;
            ...
            else
            {
                return new MaskItem<TRet, GenderedItem<TRet>?>(
                    ...
                    mask.Specific == null
                        ? null
                        : new GenderedItem<TRet>(eval(mask.Specific.Male), eval(mask.Specific.Female)));
            }
        }
            ...
            where TRetMask : class, IMask<TRet>
        {
            ...
            else
            {
                return new MaskItem<TRet, GenderedItem<TRetMask?>?>(
                    ...
                    mask.Specific == null
                        ? null
                        : new GenderedItem<TRetMask?>(maskConv(mask.Specific.Male, eval),
                            maskConv(mask.Specific.Female, eval)));
            }
        }

        public static MaskItem<bool, GenderedItem<TMask?>?>? HasBeenSetHelper<TItem, TMask>(
            IGenderedItemGetter<TItem>? item, Func<TItem, TMask?> getter)
            where TMask : class, IMask<bool>
        {
            if (item == null)
            {
        ...
        public static MaskItem<TRet, GenderedItem<MaskItem<TRet, TRetMask?>?>?>? TranslateHelper<TItem, TMask, TRet,
            TRetMask>(
            ...
            where TRetMask : class, IMask<TRet>
        {
            ...
            else
            {
                return new MaskItem<TRet, GenderedItem<MaskItem<TRet, TRetMask?>?>?>(
                    ...
                    mask.Specific == null
                        ? null
                        : new GenderedItem<MaskItem<TRet, TRetMask?>?>(
                            mask.Specific.Male == null
                                ? null
                                : new MaskItem<TRet, TRetMask?>(eval(mask.Specific.Male.Overall),
                                    maskConv(mask.Specific.Male.Specific, eval)),
                            mask.Specific.Female == null
                                ? null
                                : new MaskItem<TRet, TRetMask?>(eval(mask.Specific.Female.Overall),
                                    maskConv(mask.Specific.Female.Specific, eval))));
            }
        }
    }
}
